...
Implement only in the optimized payload and preserve existing slicer entrypoints, so previously sliced files using those entrypoints receive the behavior. Advance the package version with the version script and add matching release notes. Change upgrade-source metadata only if new guarded source provenance is introduced. Validate the package and generated contract views before any separately authorized deployment. If physical validation rejects the finish, revise the change or restore a known prior managed payload through the supported installer workflow; do not modify stock macros to compensate.
