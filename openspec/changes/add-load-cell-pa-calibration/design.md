...
**Research correction:** [corrected local firmware findings](../qualify-raw-adc-pa-capture/local-firmware-findings.md) supersede the old cache-only/raw and GPIO-passive/origin assumptions. The [raw-capture qualification change](../qualify-raw-adc-pa-capture/proposal.md) owns the next acquisition experiment. This design does not authorize enabling the existing adapters.
...
Research notes and capture evidence are maintained exclusively in [the raw-capture qualification change](../qualify-raw-adc-pa-capture/proposal.md): [reverse engineering](../qualify-raw-adc-pa-capture/reverse-engineering.md), [observations](../qualify-raw-adc-pa-capture/qidi-load-cell.md), and its `evidence/` directory.
...
The analyzed toolhead MCU firmware reports protocol version `02.02.01.08` and does not advertise a `cs1237_data` response even though the compiled Python helper registers a queue for it. `CS1237.setup_home()` arms `cs1237_setup_home` with `TriggerDispatch`/`trsync` endstop semantics and is not used for calibration. Scheduled `query_cs1237_read` commands can return `query_cs1237_data` through a temporary OID-scoped handler, but this is not a validated acquisition transaction: the firmware consumes only the OID, attempts ADC acquisition with old/empty-buffer fallback, and provides no request sequence or conversion timestamp.

The stock `query_cs1237_config_r` command is also unsuitable as a compatibility fence. Corrected static firmware analysis shows GPIO configuration-register access; safe serialization against periodic acquisition is not established. The earlier ten-clock attribution is withdrawn. A controlled idle sequence with no direct reads returned `60`, `60`, and `255`; another campaign produced equal-valued/equal-timestamp direct responses followed by a non-`60` preflight result. These observations and GPIO access make repeated configuration reads unsuitable as a passive verification fence; the physical cause of the anomalous values remains unresolved.

Capture ownership, handler cleanup, response filtering, and firmware-restart shutdown guards remain implemented and tested, but public calibration and all developer sensor commands are hard-disabled. `CS1237.read_origin_data()` is excluded as a capture candidate: corrected analysis shows repeated ADC reads and probing-reference mutation. Its historical 40 and 50 Hz results were zeroing responses, not passive samples. The existing timestamp/payload identity filter also requires replacement; transport timestamps do not identify conversions. The origin adapter exposes validation, lease acquisition, owned capture, and idempotent release primitives. A temporary source-gated coordinator exercised those primitives by acquiring before PA mutation or trapq queueing, retaining through bounded synchronous capture, owned motion completion, and temporary-state restoration, then releasing with post-capture homing-state verification. Ownership loss or unverifiable post-state forces shutdown; the installed public path does not invoke the coordinator while calibration remains disabled.

Four initial controlled captures and three subsequent ten-pulse K `0.020` campaigns requested 40 Hz polling during stationary `0.4 mm` PLA extrusion at `215 °C`. Every successful pulse preserved XYZ and logical E, restored PA and smooth time, retained QIDI Box `slot4`, and was followed by successful stock `G28`; local call-start gaps reached `74.642 ms`. Thermal soaks of 180 and 300 seconds and extending the low-flow lead from `0.15` to `0.5` seconds did not produce repeatable low-to-high amplitudes or normalized transition metrics. Two short-lead cycles inverted analyzer polarity; the long-lead cycles retained polarity but still failed amplitude, timing, tracking, and signed-area repeatability. Conversion age remains inseparable from physical pressure-response delay, so no deterministic conversion-time bound or PA sweep is accepted. `query_cs1237_begin(config=60)` is a stock reconfiguration command, not an accepted restoration path; its cache, freshness, zero-state, and post-restart behavior remain unvalidated.
...
1. A future QIDI CS1237 adapter uses bounded nonblocking raw requests, one outstanding request and an explicit deadline without host retransmission. It validates bounds and ownership, preserves all response records including equal values/timestamps, and never zeroes, reads configuration, or starts periodic polling. Existing origin-adapter code remains disabled and is not this implementation.
2. A capture coordinator retains the lease through bounded reads, owned motion completion, and temporary-state restoration; it releases ownership on every path, verifies homing state after release, and forces shutdown if post-capture state or ownership cannot be verified. Hardware qualification must characterize stale reads and timing uncertainty before using raw data for PA analysis; request/receive intervals are not conversion timestamps.
...
- **Current firmware has no advertised `cs1237_data` response** → Never arm `CS1237.setup_home()` for calibration; unqualified `query_cs1237_read` responses are characterization evidence only and are not an accepted production substitute.
...
- **Origin reads mutate probing reference; synchronous retry wrappers can send again after receiving a response** → Exclude both from capture. Test deadlines, late responses, resource bounds, and handler cleanup offline before any authorized cold-idle raw-read trial.
- **Low-flow baselines may remain pressure-history dependent** → Require repeated-cycle amplitude, polarity, and normalized-metric gates; thermal soaking and a longer lead did not stabilize K `0.020`, so the K sweep remains blocked rather than tuning around the observed drift.
...
- How often do raw reads return stale/empty data, and what timing uncertainty still permits a repeatable, print-validated PA recommendation?
- What excitation or pressure-conditioning sequence can produce repeatable low-flow baselines and normalized transition metrics? Three K `0.020` campaigns at 40 Hz, including thermal soaking and a `0.5` second lead, did not pass repeatability.
