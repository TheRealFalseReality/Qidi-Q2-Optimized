...
`reverse-engineering.md` records the analyzed host/MCU artifacts, hashes, protocol commands, homing lifecycle, firmware disassembly, rejected bulk path, live scheduled-read measurements, invalid-read evidence, Reddit claims, installer restart caveat, and remaining hardware gaps. `evidence/direct-read-cadence.json` preserves sanitized scheduled-read and idle origin-cache cadence. `evidence/origin-cache-under-force.json` preserves bounded host-call intervals and load-cell counts from stationary `0.4 mm` PLA pulses at `215 °C`. `evidence/origin-cache-repeatability.json` preserves per-run timing, signal, restoration, and analyzer metrics from three ten-pulse K `0.020` qualification campaigns.
...
1. A QIDI CS1237 adapter polls the GPIO-passive `read_origin_data()` cache at no more than 50 Hz, validates capture bounds before acquisition, acquires a process-local lease before any PA mutation or trapq queueing, records host and estimated print-time call intervals, and never reads configuration.
2. A capture coordinator retains the lease through bounded reads, owned motion completion, and temporary-state restoration; it releases ownership on every path, verifies homing state after release, and forces shutdown if post-capture state or ownership cannot be verified. Hardware validation must still bound cached-conversion age before assigning a conversion time inside each synchronous call interval.
